...
% --- Metadata ---
\title{\textbf{A Digital Humanities Approach to the Formal Reduction of the Riemann Hypothesis}\\
}
...
\begin{abstract}
We present a fully audited, kernel-verified conditional reduction of the Riemann Hypothesis (RH) to an explicit frontier problem with three coupled limits, formalized in Lean~4 with \textbf{8,626 verified build jobs} (complete kernel-verified reduction chain from Nyman--Beurling through signed right-line target, frequency split and high-tail exchange, dyadic Bettin--Chandee ledger, divisor-square bound, ultra-high tail decay, arithmetic-mass ledger, finite middle window, phase correction $\bar{u}=u \pmod{q}$, direct additive reassembly, large-sieve exponent stop test, paired-kernel analysis, Ramanujan completion-defect decomposition, GCD stratification, normalized $Lq$-superperiod cancellation, complete nested Abel decomposition, final superperiod boundary audit, Fourier representation, collision control, even-modulus aliasing, minimal RH-strength interface, exact weighted affine decomposition, and collision matching classification with LCM congruences and endpoint incidence analysis) and \textbf{zero custom axioms} in the active umbrella (\texttt{NBMellinTools}). The reduction chain spans 15 intermediate hypotheses ($H_1$--$H_{15}$) across 9 formal reduction stages, each kernel-certified. PENDING COMPREHENSIVE FORMAL AUDIT.

Rather than claiming an unconditional proof of RH, we establish a rigorous bridge from the Nyman--Beurling criterion through exact algebraic and analytic reductions, culminating in an explicit frontier problem. The methodology is anchored in a curated dataset of 498 peer-reviewed papers (21,994 knowledge graph nodes, 12,912 relations). Over 99\% of the mathematical architecture is grounded in established published literature; Large Language Models (Claude Sonnet 5, GPT-5.6 Sol, Gemini 3.1, Mistral Medium 3.5, DeepSeek-v4, Kimi K3) serve exclusively as formalization compilers and adversarial auditors under Lean~4 kernel supervision.
...
\paragraph{Supplementary Materials:} Code, documentation, and knowledge graph available at \url{https://github.com/xavierfresquet/riemann-github}. Full axiom audit, session logs, and verification artifacts publicly reproducible.
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{GitHub Repository:} \url{https://github.com/xavierfresquet/riemann-github}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item Clone repository: \texttt{git clone https://github.com/xavierfresquet/riemann-github}
...
\begin{center}
\url{https://github.com/xavierfresquet/riemann-formalization}
\end{center}
...
\begin{verbatim}
git clone https://github.com/xavierfresquet/riemann-formalization.git
cd riemann-formalization
