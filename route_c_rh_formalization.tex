...
\textbf{Main result:} The forward implication $\text{\texttt{NymanBeurlingCriterion}} \implies \text{\texttt{RiemannHypothesis}}$ and the $L^2$ Mellin--Plancherel isometry ($\int_{-\infty}^{\infty} \|\mathcal{M}g(1/2+it)\|^2 \diff t = 2\pi \|g\|^2$) are kernel-verified in Lean~4 with 0 custom axioms and 0 sorries. The full equivalence ($\text{\texttt{NymanBeurlingCriterion}} \iff \text{\texttt{RiemannHypothesis}}$) is established, with the reverse direction requiring classical Hardy space function theory (Beurling 1955, inner-outer factorization). The frontier reduction isolates the signed bilinear dispersion problem (H15) as the remaining open arithmetic target.
\end{abstract}
