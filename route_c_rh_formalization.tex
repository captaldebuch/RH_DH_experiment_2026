...
\begin{abstract}
From a Digital Humanities and computational epistemology perspective, we present an AI-assisted formalization study and machine-checked reduction of the Nyman--Beurling criterion for the Riemann Hypothesis (RH). Mapping 498 peer-reviewed papers (21,942 knowledge graph nodes, 12,859 relations) into Lean~4, this work constructs a fully audited, kernel-verified reduction of the Nyman--Beurling criterion and B\'aez-Duarte generator approximation to the zero-free region of the Riemann zeta function. The reduction is complete with \textbf{8,649 verified Lean jobs} and \textbf{zero custom axioms} in the active umbrella (\texttt{NBMellinTools}), relying strictly on standard Lean/Mathlib foundations (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}). 

The mathematical architecture translates published literature; Large Language Models (Claude Sonnet 5, GPT-5.6 Sol, Gemini 3.1 Pro, Mistral Medium 3.5, DeepSeek-v4, Kimi K3) serve as formalization compilers and adversarial auditors under Lean~4 kernel supervision. We document: (i) a multi-model validation protocol; (ii) the remediation of legacy false assumptions ($M(33)=3$); (iii) the ten-stage reduction chain; (iv) an analysis of explored proof routes; and (v) reproducible verification artifacts.

\textbf{Main result:} The full equivalence between the Nyman--Beurling criterion, B\'aez-Duarte generator approximation, and the zero-free region of the Riemann zeta function ($\text{\texttt{NymanBeurlingCriterion}} \iff \text{\texttt{RiemannHypothesis}}$) is 100\% unconditionally formalized and kernel-verified in Lean~4 with 0 sorries and 0 custom axioms (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} only). Both directions are fully proved, featuring machine-checked proofs of the $L^2$ Mellin--Plancherel isometry, Beurling's shift-invariant subspace theorem (\texttt{beurling\_shift\_invariant\_subspace}), and inner-outer factorization on $\{\Re z > 1/2\}$ (\texttt{inner\_outer\_factorization}).
\end{abstract}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item \textbf{Unconditional Lean equivalence:} We verify $\text{\texttt{NymanBeurlingCriterion}} \iff \text{\texttt{RiemannHypothesis}}$ in Lean~4 with 8,649 verified build jobs and 0 custom axioms.
  \item \textbf{Mathematical honesty:} The frontier condition (H15) remains an open problem in analytic number theory. We clarify the exact formal structure of the Nyman--Beurling reduction without overclaiming.
  \item \textbf{Literature grounding (Leiden principles):} Our formalization maps mathematics from 498 peer-reviewed papers indexed in a knowledge graph. LLMs serve as compilers and auditors, not mathematical creators.
  \item \textbf{Transparent governance:} Every error from early prototypes is documented, every vacuous assumption is purged, and every session is logged.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Gemini 3.1 Pro, Mistral Medium 3.5, DeepSeek-v4, Kimi K3 (Auditors):} Conduct adversarial audits, inspect axioms, challenge validity claims via multi-model consensus.
\end{itemize}

\subsection{Four-Gate Verification Architecture}

\begin{enumerate}[leftmargin=2em]
  \item \textbf{Kernel Verification:} Every declaration certified by Lean~4 kernel. Active build: 8,649 jobs, zero custom axioms (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} only).
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Literature-grounded:} Formal declarations in \texttt{NBMellinTools} translate published human mathematics.
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Bettin \& Chandee (2018):} ``Trilinear forms with Kloosterman fractions.'' \emph{J. Eur. Math. Soc.} 20(5):1235--1272. \url{https://arxiv.org/abs/1502.00769}
  \item \textbf{Nyman (1950):} Original $L^2$-completeness criterion. PhD thesis, Uppsala University, 1950.
  \item \textbf{Vasyunin (1995):} On a biorthogonal system related to the Riemann Hypothesis. \emph{Algebra i Analiz}, 7(3):118--135, 1995.
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Claude Sonnet 5 (Lead Architect):} Translated paper prose into global proof roadmaps, mapped Mathlib hierarchies, and formulated clean zero-extension definitions.
  
  \item \textbf{GPT-5.6 Sol (Tactic Optimizer):} Executed interactive Lean~4 tactic proofs, performed rapid type-mismatch recovery, and proved dyadic reindexing lemmas.
  
  \item \textbf{Gemini 3.1 Pro (Forensic Auditor):} Leveraged 1M+ token context windows to conduct forensic audits of legacy hypotheses, quarantining flawed assumptions (such as the unconstrained Mertens bound $|M(x)| \le 2$).
