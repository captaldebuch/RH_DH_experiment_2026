...
-/
import NBMellinTools.NB15HurwitzZeroEndpoint

/-!
# The active Estermann--Vasyunin endpoint

This file ports the finite Fourier calculation into the active NB15 namespace.
It evaluates the actual finite Hurwitz continuation at zero; no value package,
analytic assumption, or RH input is used.
-/
...
/-- The cosine companion to the rational sine coefficient. -/
...
theorem rationalCosinePeriodicTerm_eq_stdAddChar_re
  ...
  have hphase := congrArg Complex.re
    (bblsAdditiveCharacter_rat_eq_stdAddChar k q n)
  have hcast : ((k * n : ℕ) : ZMod q) =
      (k : ZMod q) * (n : ZMod q) := by
    push_cast
    rfl
  rw [hcast] at hphase
  rw [← hphase]
  simp [rationalCosinePeriodicTerm, bblsAdditiveCharacter,
    Complex.exp_re]
  ring_nf
...
theorem dft_residueCotangent
  ...
  · have hres := residueCotangent_natCast_Ioc (q := q)
    ...
    · rw [Complex.im_sum]
      ...
      have hsine := rationalSineCotangentFiniteIdentity j hj
...
theorem sum_bblsEstermannResiduePhase_mul_bernoulli
  ...
  calc
    (∑ k : ZMod q,
      ...
      apply Finset.sum_congr rfl
      intro k _
      unfold bblsEstermannResiduePhase t
      congr 1
...
/-- The active inverse-frequency Bernoulli double sum is exactly the
Vasyunin cotangent endpoint. -/
theorem bblsEstermannBernoulliFiniteValue_inverse_eq_vasyunin
    (a q : ℕ) [NeZero q] (hcop : Nat.Coprime a q) :
    bblsEstermannBernoulliFiniteValue (inverseResidue a q) q =
      (1 : ℂ) / 4 -
        Complex.I / 2 * (cotangentSumVFormula a q : ℂ) := by
  let u : (ZMod q)ˣ := ZMod.unitOfCoprime a hcop
  ...
  have hfinite :
      bblsEstermannBernoulliFiniteValue (inverseResidue a q) q =
        ∑ j : ZMod q, F j := by
    unfold bblsEstermannBernoulliFiniteValue F
    simp_rw [sum_bblsEstermannResiduePhase_mul_bernoulli]
  have hinv (r : ZMod q) :
...
/-- The genuine active Hurwitz--Estermann continuation at zero is the
Vasyunin cotangent value for an inverse reduced numerator. -/
theorem bblsInverseEstermann_zero_eq_vasyunin
    (a q : ℕ) [NeZero q] (hcop : Nat.Coprime a q) :
    bblsEstermannHurwitzContinuation (inverseResidue a q) q 0 =
      (1 : ℂ) / 4 -
        Complex.I / 2 * (cotangentSumVFormula a q : ℂ) := by
  rw [bblsInverseEstermann_zero_eq_bernoulliFinite]
  exact bblsEstermannBernoulliFiniteValue_inverse_eq_vasyunin a q hcop
