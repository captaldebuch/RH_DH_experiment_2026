...
def HardyDisc : Type :=
  { f : ℂ → ℂ // AnalyticOn ℂ f (ball 0 1) ∧ ∃ C, ∀ r ∈ Ioo (0:ℝ) 1, ∫ θ in (0..2*π), ‖f (r * exp (I * θ))‖^2 ≤ C }

/-- Inner function definition on H²(𝔻). -/
def IsInnerDisc (f : ℂ → ℂ) : Prop :=
  AnalyticOn ℂ f (ball 0 1) ∧ ∀ᵐ θ, ‖f (exp (I * θ))‖ = 1

/-- Outer function definition on H²(𝔻). -/
def IsOuterDisc (g : ℂ → ℂ) : Prop :=
  AnalyticOn ℂ g (ball 0 1) ∧ ∀ z ∈ ball (0:ℂ) 1, g z ≠ 0
