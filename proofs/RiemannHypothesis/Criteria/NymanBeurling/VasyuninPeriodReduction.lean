...
lettered plan). Proposition 21 is proved for the rational case this project
actually needs (`θ = k/h`, see `baezDuarte_prop21_rat_of_prop12`) from the
proved rational Proposition 12 lattice-point identity. Proposition 22 is also
proved below by a discrete Abel/Stieltjes summation-by-parts argument. The
unused generic real-parameter Proposition 21 declaration from the historical
research copy is deliberately omitted from this promoted rational module.
...
/- The BBLS Proposition 21 identity is required downstream only for rational
parameters `θ = k/h`. The promoted proof below works directly with
`Nat`/`Int` divisibility, including the exact-integer correction for the
Dedekind sawtooth convention. The unused generic real-parameter declaration
from the historical source is not included. -/
