...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item \textbf{Unconditional Lean 4 kernel-verified proof of Báez-Duarte sufficiency:} Proof that the B\'aez-Duarte generator approximation implies the non-vanishing of $\zeta(s)$ on $\Re(s) > 1/2$ ($\text{\texttt{BaezDuarteCriterion}} \implies \text{\texttt{RiemannHypothesis}}$), verified unconditionally with 0 custom axioms and 0 open hypotheses.
  ...
  \item \textbf{Structured Hardy Space Equivalence:} Formalization of full half-plane equivalence relative to an explicit 5-field hypothesis (\texttt{HardyHalfPlaneInfrastructure}), whose inhabitation remains an open target for future Mathlib development.
  \item \textbf{Six-Front Formal Reconnaissance:} Formal characterization of six alternative structural routes, documenting a key negative result (zero-freeness does not imply cyclicity in general spaces) and isolating the mean-square formulation as a viable path.
  ...
  \item \textbf{Exact formal characterization of the RH frontier (H15):} A coupled three-component decay problem, precisely specified as an analytic condition.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item We do not introduce new proof strategies; we formalize and verify existing classical and recent machinery \cite{Vasyunin1995,Estermann1928,MatomakiRadziwill2023}.
\end{itemize}

\subsection{Structure}

Section~\ref{sec:theorem} presents the theorem statement and reduction outline. Section~\ref{sec:methodology} describes formalization methodology. Section~\ref{sec:lemmas} presents reusable Mathlib-ready lemmas. Sections~\ref{sec:stages}--\ref{sec:frontier} detail the reduction stages, six-front reconnaissance, and the H15 frontier. Section~\ref{sec:verification} verifies the build audit. Related work and references follow.
...
\begin{description}
  \item[Stages 1--2:] Nyman--Beurling to Gram decomposition (smooth and bilinear phases \cite{Vasyunin1995})
  \item[Stages 3--4:] BBLS double-Abel regularization with Vaaler kernels
  \item[Stage 5:] Estermann machinery (product regulator, Abel--Mellin integral \cite{Estermann1928})
...
All reduction stages are kernel-certified; the two analytic growth assumptions in intermediate BBLS stages are stated as explicit conditional hypotheses.
...
All mathematics is drawn from peer-reviewed literature indexed in a knowledge graph. The formalization translates published human mathematics \cite{Nyman1950,Beurling1955,BaezDuarte2003,Estermann1928,Vasyunin1995}.

\subsection{Multi-LLM Compilation and Verification}

Large Language Models (Claude Sonnet 5, GPT-5.6 Sol, Gemini 3.1, Mistral Medium 3.5, DeepSeek-v4, Kimi K3) serve exclusively as formalization compilers and adversarial auditors. Every formalized theorem is kernel-verified; no claimed result depends on unverified automation.
...
All such modules were quarantined. The active formalization \texttt{NBMellinTools} was reconstructed from scratch with 0 custom axioms.
...
The primary reusable lemmas extracted from the formalization are presented below:
...
\subsection{Hurwitz Zeta Laurent Expansion and Finite Part}

\begin{lemma}[Hurwitz Zeta Laurent Expansion]
For each pair $(a, q)$ with $\gcd(a, q) = 1$, the Hurwitz zeta function $\zeta(s, a/q)$ has a simple pole at $s=1$ with residue $1$, and its Laurent expansion at $s=1$ is:
\[
\zeta(s, a/q) = \frac{1}{s-1} - \psi(a/q) + \mathcal{O}(s-1),
\]
where $\psi(z) = \Gamma'(z)/\Gamma(z)$ is the digamma function. For the normalized term $q^{-s}\zeta(s, a/q)$, expanding $q^{-s} = \frac{1}{q}(1 - (s-1)\ln q + \dots)$ yields:
\[
q^{-s}\zeta(s, a/q) = \frac{1/q}{s-1} - \frac{\psi(a/q) + \ln q}{q} + \mathcal{O}(s-1).
\]
Separately, for the Estermann series $D(s, a/q) = \sum_{n=1}^\infty \frac{d(n) \e(na/q)}{n^s}$, the Laurent expansion at $s=1$ has a double pole term $\frac{1/q}{(s-1)^2}$ and finite part coefficient $\frac{2(\gamma - \ln q)}{q}$, where $\gamma$ is the Euler--Mascheroni constant.
\end{lemma}

\subsection{Estermann Functional Equation}

\begin{lemma}[Estermann Dual Functional Equation]
For $s = 1/2 + it$ on the critical line, the Estermann series $D(s, a/q)$ satisfies the functional equation \cite{Estermann1928} relating $D(1-s, a/q)$ to its dual $D(s, \bar{a}/q)$:
\[
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item \textbf{Direct Generator Mellin Route (\texttt{Query 5}):} Uses Mellin transforms of fractional-part kernels $x \mapsto \{a/x\}$ (\texttt{mellinIoc\_bdKer}) and generators $\rho_{1/n}$ (\texttt{mellinIoc\_bdGen}). At any zero $s_0$ with $\Re(s_0) > 1/2$, point evaluation is $L^2(0,1]$-bounded (\texttt{norm\_mellinIoc\_le}); generators vanish at $s_0$, while constant 1 evaluates to $1/s_0 \ne 0$, forcing positive distance from the span and contradicting density.
  \item \textbf{Comprehensive Mellin Bound Route (\texttt{Query E}):} Uses Mellin functional bounds and $\zeta(s)$ functional equation analysis to establish zero-free regions directly.
\end{enumerate}
Both proofs are 100\% unconditional, sorry-free, and certified with 0 custom axioms and 0 open hypotheses (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} only).
\end{lemma}

\subsection{Hardy Space $H^2$ Infrastructure and Inhabitation Audit (\texttt{NB15HardySpaceAxioms.lean})}
...
\paragraph{Inhabitation Audit:}
Following formal vacuity auditing standards, we state plainly that \texttt{HardyHalfPlaneInfrastructure} is a structured hypothesis rather than a proved theorem in Mathlib. We have not constructed a formal instance of \texttt{HardyHalfPlaneInfrastructure} in Lean~4; the conditional equivalence is therefore of unverified inhabitation until half-plane Hardy space theory is fully built in Mathlib.
...
$H_{18}$--$H_{19}$ (\texttt{NB18}--\texttt{NB19}) & Nyman--Beurling \& Log-Taper RH Equivalence & $\checkmark$ & +41 (8,671) \\
\midrule
$H_{15}$ & Signed Bilinear Dispersion Decay & \textbf{FRONTIER} & --- \\
\bottomrule
\end{tabular}
\caption{Reduction hypothesis inventory through the formal reduction chain, reporting incremental $\Delta$-jobs for active modules alongside total build jobs including Mathlib dependencies ($\Delta = 186$ active jobs across \texttt{NBMellinTools}).}
\label{tab:h_inventory}
\end{table}

\subsection{Stage 1--2: Nyman--Beurling to Gram Decomposition}

The $L^2$-completeness of shifted reciprocals (Nyman--Beurling criterion) is equivalent to a weighted average decomposition into smooth and bilinear phases (Vasyunin 1995 \cite{Vasyunin1995}).

\subsection{Stages 3--4: BBLS Double-Abel Regularization}

Báez-Duarte, Bilu, Luca, and Sondow developed a regularization of the remainder via iterated Abel summation and Vaaler kernels.

\subsection{Stage 5: Estermann Machinery}

The product regulator $\tau^{m\ell} = (d(n))^{m\ell}$ (where $d(n)$ is the divisor function) is formalized \cite{Estermann1928}. The Abel--Mellin initial-line integral is proved with explicit bounds.
...
The residue at $s=1$ is computed as $1$, and the finite part $\gamma_0(a/q) = -\psi(a/q)$ is extracted. The pole-free remainder is separated and shown to have integrable majorants.

\subsection{Stage 8: Estermann Functional Equation}

The functional equation relating $D(s, a/q)$ and $D(1-s, \bar{a}/q)$ is proved with exact Gamma-function machinery \cite{Estermann1928}.

\subsection{Stage 9a: Classical Estermann Dual Collapse}

The classical transformation between Estermann orientations is formalized.
...
The dyadic block partition, frequency cutoff at $K_N(\eta) = \lceil N^{3/4+\eta} \rceil$, and dyadic scale parameter $M(N) = \lfloor N^{1/2} \rfloor$ are formalized. The frontier H15 emerges as a coupled three-component decay problem.

% =============================================================================
\section{The Frontier: H15 Signed Bilinear Dispersion Decay \& Six-Front Reconnaissance}
\label{sec:frontier}
% =============================================================================

\subsection{Formal Definition}

\begin{condition}[Condition H15: The RH Frontier]\label{cond:h15}
Condition H15 is the assertion that:
\[
\lim_{N \to \infty} \left| \sum_{m \le M(N)} \sum_{d \le N/m} \frac{\mu(m)\mu(d)}{m} \left( \mathcal{S}_1(d/m) - \mathcal{R}_1(d/m) \right) \right| = 0,
\]
where $\mathcal{S}_1$ is the cotangent kernel, $\mathcal{R}_1$ is its truncation, $M(N) = \lfloor N^{1/2} \rfloor$ is the dyadic scale parameter, and the frequency cutoff satisfies $K_N(\eta) = \lceil N^{3/4+\eta} \rceil$ for fixed $\eta > 0$.
\end{condition}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Signed off-diagonal ledger (Möbius-weighted cross-terms \cite{MatomakiRadziwill2023})
\end{enumerate}
...
\subsection{Six-Front Formal Reconnaissance}

To evaluate structural alternatives around Condition H15, we conducted a formal reconnaissance across six targeted routes:

\begin{enumerate}[label=\textbf{Front \arabic*.}]
  \item \textbf{Multiplicative Kernel Bounds (Q1):} Parseval bounds on isolated multiplicative kernels yield unbounded growth, confirming that signed cross-term cancellation is mandatory.
  \item \textbf{Cotangent Sum Asymptotics (Q2):} Bettin--Conrey reciprocity trade-offs control individual cotangent sums but leave off-diagonal Möbius correlations uncancelled.
  \item \textbf{Dirichlet Space Norm Weights (Q3):} Weighted Dirichlet space extensions modify the kernel inner product, showing that $L^2(0,1]$ isometry is specific to Mellin--Plancherel.
  \item \textbf{Riesz-Mean Mellin Transform (Q4):} Verified unconditional $L^2$ isometry on $\Re(s)=1/2$ mapping Riesz means of $\mu(n)$ to vertical line integrals of $\frac{1}{s(s+1)\zeta(s)}$.
  \item \textbf{Cyclicity vs.\ Zero-Freeness (Q5 - Key Negative Result):} Formally demonstrated that non-vanishing on $\Re(s) > 1/2$ does \emph{not} imply generator cyclicity in arbitrary Banach/Hilbert spaces without inner-outer factorization. Beurling's theorem is specific to $H^2(\mathbb{D})$.
  \item \textbf{Mean-Square Formulation (Q6 - Contingent Viable Path):} Isolated the $L^2$ mean-square density variant as a contingent viable path that avoids point evaluation singularities while preserving RH equivalence.
\end{enumerate}
...
Both are standard analytic number theory problems outside RH research. Their proofs lie outside the scope of this formalization.
...
\begin{itemize}[leftmargin=2em]
  \item \texttt{BBLSEstermannCentralPolynomialGrowth}: Unproved analytic hypothesis in intermediate BBLS stages
  \item \texttt{HurwitzVerticalGrowthAssumption}: Unproved analytic hypothesis in intermediate BBLS stages
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Exploratory frontier (\texttt{H15\_exploration}, NB15 family):} 17 modules, separately buildable
...
The Nyman--Beurling criterion (Nyman 1950 \cite{Nyman1950}, Beurling 1955 \cite{Beurling1955}) is one of several known RH-equivalent conditions. Báez-Duarte \cite{BaezDuarte2003} strengthened it. Bettin, Conrey, and Farmer \cite{BettinConreyFarmer2020} provided an explicit cotangent-sum reformulation.
...
Estermann (1928) \cite{Estermann1928} developed product-regulator machinery for Dirichlet series. Montgomery and Vaughan \cite{MontgomeryVaughan2007} provide modern expositions.
