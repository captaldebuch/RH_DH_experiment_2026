...
The work contributes reusable Mathlib-ready lemmas on fractional-part Mellin transforms, Riesz-mean Mellin transforms, Laurent finite-part extraction, Estermann functional equations, and Mellin--Plancherel isometries. We also report findings on the H15 frontier condition as a conditionally connected bridge requiring two explicit unproved hypotheses: \texttt{H15CoupledVariationBoundaryDecay} and \texttt{H15PointwiseAggregateToLogTaperTransfer}. We conclude that the transfer proposition (\texttt{H15PointwiseAggregateToLogTaperTransfer}) is the immediate next frontier for conditional progress.
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  \item \textbf{H15 Conditional Bridge Architecture:} Characterization of Condition H15 as a conditionally connected bridge requiring two explicit unproved hypotheses (\texttt{H15CoupledVariationBoundaryDecay} and \texttt{H15PointwiseAggregateToLogTaperTransfer}).
...
Section~\ref{sec:theorem} presents the theorem statement and reduction outline. Section~\ref{sec:methodology} describes formalization methodology. Section~\ref{sec:lemmas} presents reusable Mathlib-ready lemmas. Sections~\ref{sec:stages}--\ref{sec:frontier} detail the reduction stages, the H15 conditional bridge, and open transfer hypotheses. Section~\ref{sec:verification} verifies the build audit. Related work and references follow.
...
% =============================================================================
\section{The Frontier: H15 as a Conditional Bridge}
\label{sec:frontier}
% =============================================================================
...
\subsection{H15 as a Conditional Bridge Architecture}

Rather than being an isolated unproved assumption, Condition H15 is formalized as a \emph{conditionally connected bridge} between discrete Möbius arithmetic and continuous Mellin density. Completing this bridge requires two explicit, named hypotheses:
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item \texttt{H15CoupledVariationBoundaryDecay}: Bounding the coupled variation of signed Möbius-weighted cross-terms along contour boundaries.
  \item \texttt{H15PointwiseAggregateToLogTaperTransfer}: Transporting pointwise aggregate decay of cotangent sums to $L^2(0,1]$ density of log-taper generators $\rho_a(x)$.
\end{enumerate}

Both declarations are explicitly disclosed in \texttt{NB20H15RHBridge.lean}. The Lean kernel type-checks the conditional deduction:
\[
\text{\texttt{H15CoupledVariationBoundaryDecay}} \land \text{\texttt{H15PointwiseAggregateToLogTaperTransfer}} \implies \text{\texttt{RiemannHypothesis}}.
\]

\subsection{The Transfer Proposition as the Immediate Frontier}

Our formalization demonstrates that the transfer proposition (\texttt{H15PointwiseAggregateToLogTaperTransfer}) is the immediate bottleneck for conditional progress. Bounding the pointwise aggregate alone is insufficient without the transfer operator to log-taper $L^2$ norms. Proving this transfer operator is the primary target for future work.

\subsection{Two Analytic Growth Hypotheses}

Two auxiliary analytic bounds in intermediate BBLS stages are stated as explicit conditional hypotheses:
\begin{itemize}[leftmargin=2em]
...
\end{itemize}
 ...
 Both are standard analytic number theory problems outside RH research.
...
This work provides a kernel-verified Lean 4 formalization of the Nyman--Beurling criterion, proving Báez-Duarte sufficiency unconditionally and establishing Condition H15 as a conditionally connected bridge to RH. We disclose two explicit open bridge hypotheses (\texttt{H15CoupledVariationBoundaryDecay} and \texttt{H15PointwiseAggregateToLogTaperTransfer}), identifying the transfer proposition as the immediate next frontier for conditional formal progress.
